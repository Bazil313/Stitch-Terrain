...
public class CustomThreadPool
{
	...
	#region configurable items - for now have these as constants
	private const int MAX = 4; // maximum no of threads in pool
	...
	private const int CLEANUP_INTERVAL = 6000; // millisecond - to free waiting threads in pool
	private const int SCHEDULING_INTERVAL = 5; // millisecond - look for task in queue in loop
	...
	private bool countAddToQueue = false;
	private float totalAddedToQueue = 0;
	private bool emptyThreadPool = false;
		...
		taskScheduler = new Thread(() =>
			{
								...
								foreach (TaskItem ti in Pool) // while reading the pool another thread should not add/remove to that pool
								{
									lock (ti) // locking item
									{
										...
										{ // if in the Pool task state is completed then a different task can be handed over to that thread
											//Debug.Log("Take from pool: " + ti.taskHandle.Token.ID);
											ti.taskHandle = readyItem;
											//Debug.Log("Overwrite from queue: " + ti.taskHandle.Token.ID);
				...
				} while (!emptyThreadPool);
			});
		taskScheduler.Priority = System.Threading.ThreadPriority.AboveNormal;
		taskScheduler.Start();
	}
	...
	private void AddTaskToPool(TaskItem taskItem)
	{
							...
							// thread as taskItem itself is is mapped to a dedicated thread]
						{
							if (taskItem.taskHandle.callback != null && taskItem.taskState != TaskState.aborted)
							{
								try
								{
									...
									//Debug.Log("Marking Complete: " + taskItem.taskHandle.Token.ID);
		...
		{
			Pool.Add(taskItem);
			//Debug.Log ("Pool increased to " + getPoolCount () + ", new task: " + taskItem.taskHandle.Token.ID);
		}
	}
	...
	private void CleanupPool()
	{
		...
		lock (criticalLock)
		{
			if(!emptyThreadPool) filteredTask = Pool.Where(ti => ti.taskState == TaskState.aborted).ToList();
			else foreach (TaskItem t in Pool) CancelUserTask (t.taskHandle.Token);
			...
			foreach (var taskItem in filteredTask) // clean all aborted thread
			{
				try
				{
					...
					taskItem.handler.Priority = System.Threading.ThreadPriority.Lowest;
			...
			if (total > MIN) // clean waiting threads over minimum limit
			{
				...
				foreach (var taskItem in filteredTask)
				{
					taskItem.handler.Priority = System.Threading.ThreadPriority.AboveNormal;
					taskItem.taskState = TaskState.aborted;
					Pool.Remove(taskItem);
					total--;
					if (total == MIN && !emptyThreadPool) break;
				}
			}
			while (Pool.Count < MIN && !emptyThreadPool)
			{
				TaskItem ti = new TaskItem() { taskState = TaskState.notstarted };
				ti.taskHandle = new TaskHandle() { task = () => { } };
				ti.taskHandle.Token = new ClientHandle() { ID = Guid.NewGuid() };
	...
	public float getTotalAddedToQueue()
	{
		return totalAddedToQueue;
	}

	public void resetTotalAddedToQueue()
	{
		totalAddedToQueue = 0;
	}

	public void setToCountQueueAdding(bool set)
	{
		countAddToQueue = set;
	}
	...
	public void CancelUserTask(ClientHandle clientToken)
	{
		lock (syncLock)
		{
			var thandle = ReadyQueue.FirstOrDefault(th => th.Token.ID == clientToken.ID);
			...
			else // in case thread is running the task - try aborting the thread to cancel the operation (rude behavior)
			{
				...
				lock (criticalLock)
				{
					taskItem = Pool.FirstOrDefault(task => task.taskHandle.Token.ID == clientToken.ID);
				}
				if (taskItem != null)
				{
					lock (taskItem) // only item need the locking
					{
						...
						{
							taskItem.taskState = TaskState.aborted;
							Debug.Log("Task Aborted: " + taskItem.taskHandle.Token.ID );
						...
						if (taskItem.taskState == TaskState.aborted) // this does not need criticalLock
						{
							try
							{
								...
								taskItem.handler.Priority = System.Threading.ThreadPriority.BelowNormal;
								taskItem.handler.IsBackground = true;
							}
							catch { }
						}
